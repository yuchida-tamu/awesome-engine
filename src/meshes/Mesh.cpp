...
void Mesh::initialize()
{
    ...
    {
        std::cerr << "Warning: Attempting to initialize mesh with empty vertex or index data." << std::endl;
        return;
    }
    ...
    // Bind VAO - all subsequent buffer operations will be associated with this VAO
    glBindVertexArray(m_VAO);
    ...
    // IMPORTANT: EBO must be bound while VAO is active to become part of VAO state
    ...
    // Note: VBO and EBO remain bound globally, but this is fine since they're not used directly
    // The VAO stores references to them, so we can unbind them if desired (optional)
