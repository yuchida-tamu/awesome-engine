...
void TextureLoader::Clear() {
  if (!m_textures.empty()) {
    ...
    // キャストが必要
    glDeleteTextures(static_cast<GLsizei>(m_textures.size()), // テクスチャの数
...
void TextureLoader::Load2D(GLuint *texture, std::string &filepath) {
  ...
  if (*texture == 0) {
    std::cerr << "Error: Failed to generate texture object for: " << filepath
              << std::endl;
    return;
  }
  ...
  if (textureData) {
    if (width <= 0 || height <= 0) {
      std::cerr << "Error: Invalid texture dimensions for: " << filepath
    ...
    std::cerr << "Error: Failed to load texture image: " << filepath
              << std::endl;
  }
  stbi_image_free(textureData);
  glBindTexture(GL_TEXTURE_2D, 0);
