...
void Shader::AddShader(const std::string &filepath, GLenum shaderType) {
  ...
  if (!m_success) {
    ...
    std::string errorMsg =
        "Shader compilation failed for " + filepath + ":\n" + m_infoLog;
...
void Shader::UseProgram() {
  if (m_programId == 0) {
    std::cerr << "Error: Attempting to use a shader that has not been linked "
                 "or failed to link."
              << std::endl;
    return;
  }
...
void Shader::SetUniformInt(const std::string &name, int value) {
  ...
  if (location != -1) {
    ...
    std::cerr << "Warning: Uniform '" << name
              << "' not found or not used in shader." << std::endl;
  }
}
...
void Shader::SetUniformVec2(const std::string &name, const float *ptr) {
  GLint location = GetUniformLocation(name);
...
Shader::Shader(Shader &&other) noexcept
    : m_shaderIds(std::move(other.m_shaderIds)), m_programId(other.m_programId),
